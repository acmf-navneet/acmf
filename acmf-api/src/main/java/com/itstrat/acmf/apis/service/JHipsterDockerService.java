...
public class JHipsterDockerService {
     ...
     * in the project directory so that "jhipster jdl" can generate the app with domain model.
     *
     * @param dir     The directory where the JDL file should be created.
     * @param content The full JDL content (e.g. application { config { ... } } and entity blocks).
     * @throws IOException If there is an error writing the file.
     */
    private void writeJdlFile(File dir, String content) throws IOException {
        java.nio.file.Files.writeString(new File(dir, JDL_FILENAME).toPath(), content);
    }
...
//    }
    ...
    private String resolveMountPath(File appDir) {
        String hostRootPath = System.getenv("HOST_ROOT_PATH");
        if (hostRootPath == null || hostRootPath.isEmpty()) {
    ...
    private void runDockerCommand(File appDir, String dockerCmd) throws IOException, InterruptedException {
        ...
        ProcessBuilder pb = os.contains("win")
                ...
                : new ProcessBuilder("/bin/bash", "-c", dockerCmd);
        pb.redirectErrorStream(true);
        if (!isDockerInstalled()) {
            throw new RuntimeException("Docker is not installed or not in PATH. Please install Docker and try again.");
        }
